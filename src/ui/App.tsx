...
//
    ...
    const startExtraction = async (injectedStream?: ReadableStream<Uint8Array>, injectedSourceInfo?: string) => {
        ...
        try {
            let source: File | ReadableStream<Uint8Array> = fileRef.current!;
            
            // If the user provided a network URL, the CONSUMER performs the fetch
            // and passes the naked ReadableStream directly into the engine.
            if (!fileRef.current && videoUrlInput) {
                setStatus('Fetching stream from network...');
                const res = await fetch(videoUrlInput);
                if (!res.ok) throw new Error("HTTP error " + res.status);
                source = res.body!;
                
                // The engine only uses this for percentage calculation if we provide it somehow... wait, we removed size from worker. 
                // We'll just be happy with MB streamed for now.
            }
